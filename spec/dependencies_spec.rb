...
describe Halite::Dependencies do
  ...
  describe '#extract_gems_from_dependencies' do
    ...
    context 'with a non-halite runtime dependency and halite_use_gem_dependencies not enabled in metadata' do
      let(:gemspec) { fake_gem {|s| s.add_dependency 'gem1' } }
      it { is_expected.to eq [] }
    end

    context 'with a halite-ish dependency and halite_use_gem_dependencies enabled in metadata' do
      let(:gemspec) { fake_gem {|s| s.add_dependency 'gem3'; s.metadata['halite_use_gem_dependencies'] = 'true' } }
      it { is_expected.to eq [] }
    end

    context 'with a development dependency and halite_use_gem_dependencies enabled in metadata' do
      let(:gemspec) { fake_gem {|s| s.add_development_dependency 'gem3'; s.metadata['halite_use_gem_dependencies'] = 'true' } }
      it { is_expected.to eq [] }
    end

    context 'with a non-halite runtime dependency and halite_use_gem_dependencies enabled in metadata' do
      let(:gemspec) { fake_gem {|s| s.add_dependency 'gem1'; s.metadata['halite_use_gem_dependencies'] = 'true' } }
